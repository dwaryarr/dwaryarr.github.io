        ...
        <div className="mx-auto grid max-w-5xl gap-10 lg:grid-cols-[1fr_240px]">
          <div>
            ...
            {post.coverImage && (
              <ScrollReveal
                delay={0.1}
                className="glass-card mt-8 overflow-hidden rounded-2xl">
                <img
                  src={post.coverImage}
                  alt={post.title}
            ...
            <ScrollReveal
              delay={0.15}
              className="prose prose-invert mt-10 max-w-none prose-headings:font-display prose-headings:font-semibold prose-a:text-[var(--accent)] prose-strong:text-white">
              <ReactMarkdown
            ...
            <div className="mt-12 grid gap-4 border-t border-white/10 pt-8 sm:grid-cols-2">
              {prev && (
                <Link
                  to={`/blog/${prev.slug}`}
                  className="glass-card rounded-xl p-4 hover:border-white/20">
                  <p className="mb-1 flex items-center gap-1 text-xs text-white/40">
                    <ArrowLeft size={12} /> {t("blog.prev")}
                  </p>
              ...
              {next && (
                <Link
                  to={`/blog/${next.slug}`}
                  className="glass-card rounded-xl p-4 text-right hover:border-white/20 sm:col-start-2">
                  <p className="mb-1 flex items-center justify-end gap-1 text-xs text-white/40">
                    {t("blog.next")} <ArrowRight size={12} />
                  </p>
